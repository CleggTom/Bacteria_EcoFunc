...
p <- read.csv("results/meanfield_simulations/symmetric_matrix/temp_symmetric_sims/params.csv",header = F) %>%
  spread("V1","V2")
...
filenames <- list.files("results/meanfield_simulations/symmetric_matrix/temp_symmetric_sims")
...
for(i in 1:length(filenames)){
  ...
  df <- read.csv(paste("results/meanfield_simulations/symmetric_matrix/temp_symmetric_sims/",filenames[i],sep = '/'),header = F)
  R <-  read.csv(paste("results/meanfield_simulations/symmetric_matrix/temp_symmetric_sims",paste0("Resp_",filenames[i]),sep = '/'),header = F)
...
plot_sim <- function(i){
 read.csv(paste("results/meanfield_simulations/symmetric_matrix/temp_symmetric_sims/",filenames[i],sep = '/'),header = F) %>%
    mutate(t = 1:n()) %>%
