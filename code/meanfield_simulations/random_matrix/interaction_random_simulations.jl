...
nSp = 150 #number of species in each simulation
nSim = 10 #number of simulations per temperature
nTemp = 10 #number of temperatures to simuluate at
na = 10 #number of interaction strengths to simulate at
...
results = Array{Any,2}(undef,nTemp*na,nSim)
...
U0  = 300.0 ; R0  = 100.0
a0_min = 0.1; a0_max = 10.0
a_vec = range(a0_min,stop=a0_max,length = na) ./ nSp
...
writedlm("results/meanfield_simulations/random_matrix/interaction_random_sims/params.csv",params,',')
...
for a = 1:na
    ...
    for T = 1:nTemp
        ...
        for sim = 1:nSim
            println("Int: ",a0," Temp: ",T_vec[T], " Sim: ",sim)
            #get actual rates
            U = rand(dU,nSp)
            R = rand(dR,nSp)
            a = rand(da,nSp,nSp)
            ...
            dir = string("results/meanfield_simulations/random_matrix/interaction_random_sims/Int_",string(a0),"_Temp_",string(T_vec[T]),"_sim_",string(sim),".csv")
            writedlm(dir,sol.u, ',')

            dir = string("results/meanfield_simulations/random_matrix/interaction_random_sims/Resp_Int_",string(a0),"_Temp_",string(T_vec[T]),"_sim_",string(sim),".csv")
            writedlm(dir,R, ',')
        end
    end
end
